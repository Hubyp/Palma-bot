...
module.exports.execute = async (client, interaction) => {
    ...
    if (numWarnings >= 5) {
        ...
        await member.kick(`You have received 3 warnings for: ${reason}.`);
        warns.delete(user.id);

        const embed = new MessageEmbed()
            .setColor('#ff0000')
            .setTitle(`${user.username} has been kicked from the server!`)
            .setDescription(`They received 3 warnings for: ${reason}.`)
        await interaction.reply({ embeds: [embed], ephemeral: true });

        const dmEmbed = new MessageEmbed()
            .setColor('#ff0000')
            .setTitle(`You have been kicked from the server!`)
            .setDescription(`You received 3 warnings for: ${reason}.`)
